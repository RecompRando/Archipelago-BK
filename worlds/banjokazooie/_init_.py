...
class BanjoKazooieWorld(World):
    ...
    def initialize_notes(self) -> None:
        for note, data in mm_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
        for note, data in ttc_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
        for note, data in cc_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
        for note, data in bs_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
        for note, data in fp_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
        for note, data in gv_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
        for note, data in mmm_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
        for note, data in rbb_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
        for note, data in ccw_notes_location_table:
            self.multiworld.get_location(note, self.player).place_locked_item(self.create_item("Note"))
